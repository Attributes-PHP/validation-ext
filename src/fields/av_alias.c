...
zend_class_entry *AV_Fields_Alias_ce;
...
ZEND_METHOD(AV_Fields_Alias, __construct)
{
    ...
    zend_update_property_string(AV_Fields_Alias_ce, Z_OBJ_P(getThis()), "name", sizeof("name") - 1, ZSTR_VAL(name));
}
...
void av_register_Alias_class(void)
{
    ...
    zend_declare_property_null(AV_Fields_Alias_ce, "name", sizeof("name") - 1, ZEND_ACC_PUBLIC);

    /* Register as an internal attribute that can be used on properties and parameters */
    zend_internal_attribute_register(AV_Fields_Alias_ce, ZEND_ATTRIBUTE_TARGET_PROPERTY | ZEND_ATTRIBUTE_TARGET_PARAMETER);
}