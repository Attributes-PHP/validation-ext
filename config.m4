...
if test "$PHP_VALIDATION_EXT" != "no"; then
  ...
  PHP_NEW_EXTENSION(validation_ext, validation_ext.c src/validate_function.c src/call_function.c src/base_model.c src/arrayof.c src/exception.c src/model_configs.c src/helpers/options.c, $ext_shared)
fi
