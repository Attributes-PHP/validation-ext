...
if test "$PHP_ATTRIBUTES_VALIDATION" != "no"; then
  ...
  PHP_NEW_EXTENSION(attributes_validation, attributes_validation.c src/validate_function.c src/call_function.c src/base_model.c src/exception.c src/model_configs.c src/helpers/av_wrappers.c src/helpers/options.c src/helpers/av_string.c src/fields/field.c src/fields/alias.c, $ext_shared)
fi
